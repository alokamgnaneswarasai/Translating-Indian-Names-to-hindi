...
# %%
STUDENT_SAP_NAME  = "ALOKAM_GNANESWARA_SAI"
STUDENT_SR_NUMBER = "22582"
...
class Tokenizer:
    ...
    def train(self, data, vocab_size):
        ...
        data = [list(name) for name in data]
        
        merges = vocab_size - len(self.vocab) + 4
        for _ in range(merges):
            pair_counts = self.compute_pair_counts(data)
            pair = max(pair_counts, key=pair_counts.get)
            new_token = pair[0] + pair[1]
            data = self.merge_pair(data, pair)            
    ...
    def compute_pair_counts(self, data):
       pair_counts={}
       for name in data:
    ...
    def merge_pair(self, data, pair):
        ...
        for name in data:
            ...
            i = 0
            while i < len(name):
                if i < len(name)-1 and name[i] == pair[0] and name[i+1] == pair[1]:
                    new_name.append(pair[0] + pair[1])
                    i += 2
                else:
    ...
    def pad(self, tokens, length):
        ...
        if len(tokens) < length:
            tokens = tokens + [2] * (length - len(tokens))
        return tokens
    ...
    def unpad(self, tokens):
        ...
        unpadded_tokens = [token for token in tokens if token != 2]
    ...
    def get_special_tokens(self):
        ...
        # ADD YOUR CODE HERE
        special_tokens = {'<s>': 0, '</s>': 1,'<pad>': 2, '<unk>': 3}
    ...
    def decode(self, tokens, strip_special=True):
        ...
        for i in range(len(tokens)):
            if tokens[i]!=self.vocab['<pad>'] :
                new_tokens.append(tokens[i])
        
        token_to_id = {v: k for k, v in self.vocab.items()}
        
        decoded_tokens = [token_to_id[token] for token in new_tokens]
        decoded_string = ''.join(decoded_tokens)
        if strip_special:
            decoded_string = decoded_string.replace('<s>', '').replace('</s>', '').replace('<pad>', '').replace('<unk>', '')
...
# ADD YOUR CODE HERE
english_data = train_data['Name'].tolist()
hindi_data = train_data['Translation'].tolist()
src_tokenizer.train(english_data, SRC_VOCAB_SIZE)
tgt_tokenizer.train(hindi_data, TGT_VOCAB_SIZE)
...
class RNNEncoderDecoderLM(torch.nn.Module):
    ...
    def __init__(self, src_vocab_size, tgt_vocab_size, embd_dims, hidden_size, num_layers=1, dropout=0.1):
        ...
        self.src_embedding = nn.Embedding(src_vocab_size, embd_dims)
        self.tgt_embedding = nn.Embedding(tgt_vocab_size, embd_dims)
        self.encoder_rnn = nn.GRU(embd_dims, hidden_size, num_layers=num_layers, dropout=dropout, batch_first=True)
        self.decoder_rnn = nn.GRU(embd_dims, hidden_size, num_layers=num_layers,dropout=dropout, batch_first=True)
    ...
    def forward(self, inputs, decoder_inputs, decoder_hidden_state=None):
        ...
        tgt_embedded = self.tgt_embedding(decoder_inputs)
        tgt_embedded = self.dropout(tgt_embedded)
       
        if decoder_hidden_state is None:
            src_embedded = self.src_embedding(inputs)
            src_embedded = self.dropout(src_embedded)
            _, encoder_hidden = self.encoder_rnn(src_embedded)
            decoder_hidden_state = encoder_hidden  
            
        
        decoder_outputs, decoder_hidden_state = self.decoder_rnn(tgt_embedded, decoder_hidden_state)
    
        output_logits = self.output_layer(decoder_outputs)
        return nn.functional.log_softmax(output_logits,dim=-1), decoder_hidden_state
    ...
    def log_probability(self, seq_x, seq_y):
        ...
        seq_x_embedded = self.src_embedding(seq_x)
        _, encoder_hidden = self.encoder_rnn(seq_x_embedded)
        
        decoder_hidden_state = encoder_hidden
        
        log_prob=0
        for i in range(seq_y.size(0)-1):
            decoder_output,decoder_hidden_state= self.forward(seq_x, seq_y[i].unsqueeze(0), decoder_hidden_state)
            print(decoder_output.shape)
            output_logits = decoder_output.squeeze(0)
            print(f"Output logits shape: {output_logits.shape}")
            log_prob += output_logits[seq_y[i+1]].item()
...
class RNNEncoderDecoderTrainer(Trainer):
    ...
    def train_step(self, x_batch, y_batch):
        ...
        for i in range(y_batch.shape[1]-1):
            ...
            y=y_batch[:,i:i+1]            
            output_logits, decoder_hidden_state = self.model(x_batch, y, decoder_hidden_state)
            output_logits=output_logits.squeeze(1)
            y1=y_batch[:,i+1:i+2].squeeze(1)
            loss += self.criterion(output_logits, y1)
        ...
        with torch.no_grad():
            for x_batch, y_batch in validation_dataloader:
                ...
                for i in range(y_batch.shape[1]-1):
                    ...
                    output_logits, decoder_hidden_state = self.model(x_batch, y, decoder_hidden_state)
                    output_logits=output_logits.squeeze(1)
                    loss += self.criterion(output_logits, y_batch[:,i+1:i+2].squeeze(1))
...
def rnn_greedy_generate(model, seq_x, src_tokenizer, tgt_tokenizer, max_length):
    ...
    model.eval()
    with torch.no_grad():
        inputs = src_tokenizer.encode(seq_x , add_start=True, add_end=True)
        ...
        for i in range(max_length):
            if decoder_hidden_state is None:
                decoder_inputs = torch.tensor([[0]]).to(model.device)

            outputs, decoder_hidden_state = model(inputs, decoder_inputs,decoder_hidden_state)            
            maxi= torch.argmax(outputs, dim=-1)
            
            if(maxi.item()==1):
                break
            
            predictions.append(maxi.item())
            decoder_inputs = maxi
    
    return tgt_tokenizer.decode(predictions)
...
class AttentionModule(torch.nn.Module):
    ...
    def __init__(self, input_size):
        ...
        # self.W=nn.Linear(input_size*2, input_size)
        # self.V=torch.nn.Parameter(torch.FloatTensor(input_size))
        
        # self.hidden_size = input_size
        # self.fc = nn.Linear(input_size, input_size,bias = False)
        # self.weight = nn.Parameter(torch.FloatTensor(input_size))
        
        self.M1 = nn.Linear(input_size, input_size)
        self.M2 = nn.Linear(input_size, input_size)
    ...
    def forward(self, encoder_outputs, decoder_hidden_state):
        ...
        # ADD YOUR CODE HERE
        # decoder_hidden_state= decoder_hidden_state.expand(encoder_outputs.shape[1],-1,-1).permute(1,0,2)
        # energy=torch.tanh(self.W(torch.cat((decoder_hidden_state,encoder_outputs),dim=2)))
        ...
        # out = torch.tanh(self.fc(decoder_hidden_state +encoder_outputs))
        # alignment_scores= torch.matmul(out, self.weight)
        # attention_scores = nn.functional.softmax(alignment_scores, dim=1).unsqueeze(1)
        # return attention_scores
        
        
        decoder_hidden_state = decoder_hidden_state.permute(1, 0, 2)
        scores = self.V(torch.tanh(self.M1(encoder_outputs) + self.M2(decoder_hidden_state)))  
...
class RNNEncoderDecoderLMWithAttention(torch.nn.Module):
    ...
    def __init__(self,src_vocab_size, tgt_vocab_size, embd_dims, hidden_size, num_layers=1, dropout=0.1):
        ...
        self.src_embedding = nn.Embedding(src_vocab_size, embd_dims)
        self.tgt_embedding = nn.Embedding(tgt_vocab_size, embd_dims)
        self.encoder_rnn = nn.GRU(embd_dims, hidden_size, num_layers=num_layers,  batch_first=True)
        self.decoder_rnn = nn.GRU(embd_dims, hidden_size, num_layers=num_layers, batch_first=True)
        self.output_layer = nn.Linear(hidden_size, tgt_vocab_size)
    ...
    def attentions(self, seq_x, terminate_token, max_length):
        ...
        embedded_inputs = self.src_embedding(seq_x.unsqueeze(0))
        encoder_outputs, encoder_hidden_state = self.encoder_rnn(embedded_inputs)
        ...
        outputs = []
        for i in range(max_length):
            decoder_input_embedded = self.tgt_embedding(decoder_input)
            attention_score = self.attention(encoder_outputs, decoder_hidden_state)
            context_vector = torch.matmul(attention_score, encoder_outputs)
            context_vector = context_vector.permute(1,0,2)
           
            context_vector = context_vector + decoder_hidden_state

            decoder_output, decoder_hidden_state = self.decoder_rnn(decoder_input_embedded, decoder_hidden_state)
            output_logits = self.output_layer(decoder_output)
            
            attention_score = attention_score.squeeze(1)
            attentions.append(attention_score.squeeze(0))
            
            best_output = torch.argmax(output_logits, dim=-1)
            
            # print(best_output.shape)
            outputs.append(best_output.item())
            
            decoder_input = best_output
        
            if best_output.item() == terminate_token:
                break
        attentions = torch.stack(attentions, dim=0).to(self.device)
    
        outputs = torch.tensor(outputs, device=self.device)
        return attentions.to('cpu'), outputs.to('cpu')
    ...
    def forward(self, inputs, decoder_inputs=None, decoder_hidden_state=None, output_attention=False):
        ...
        embedded_inputs = self.src_embedding(inputs)  # [batch_size, src_seq_len, embd_dims]
        
        embeeded_inputs = self.dropout(embedded_inputs)
        encoder_outputs, encoder_hidden_state = self.encoder_rnn(embedded_inputs)
        if decoder_inputs is None: 
            decoder_inputs = torch.zeros(inputs.size(0), 1).long().to(inputs.device)
            
        decoder_hidden_state = encoder_hidden_state if decoder_hidden_state is None else decoder_hidden_state
        
        decoder_input = self.tgt_embedding(decoder_inputs)  # [batch_size, 1, embd_dims]
        ...
        decoder_hidden_state = context_vector + decoder_hidden_state
        # decoder_hidden_state = torch.cat((context_vector, decoder_hidden_state), dim=2)
        
        # decoder_input_combined = torch.cat((decoder_hidden_state, context_vector), dim=2)
        decoder_input_combined = decoder_input
        
        decoder_output, decoder_hidden_state = self.decoder_rnn(decoder_input_combined, decoder_hidden_state)
        
        output_logits = self.output_layer(decoder_output)
        
        decoder_outputs= nn.functional.log_softmax(output_logits,dim=-1)

        if output_attention:
            return decoder_outputs, decoder_hidden_state, attention_score
...
with torch.no_grad():
    for i, row in train_data.sample(n=4, random_state=69, ignore_index=True).iterrows():
        ...
        attentions, tgt_tokens = model.attentions(src_tokens, tgt_tokenizer.get_special_tokens()['</s>'], max_length=50)
...
def rnn_better_generate(model, seq_x, src_tokenizer, tgt_tokenizer, max_length=18, k=4, length_penalty_alpha=0.8):
    """ Given a source string, translate it to the target language using the trained model.
    ...
    model.eval()
    with torch.no_grad():
        seqx = src_tokenizer.encode(seq_x, add_start=True, add_end=True)
    
        seqx = torch.tensor(seqx).unsqueeze(0).to(model.device)
        decoder_hidden_state = None
        
        beam_candidates = [(0,[0],decoder_hidden_state)]  
        
        for _ in range(max_length):
            new_candidates =[]
            for candidate_probs , candidate_seq ,decoder_hidden_state in beam_candidates:
                
                if candidate_seq[-1]==1:
                    new_candidates.append((candidate_probs,candidate_seq,decoder_hidden_state))
                    continue

                decoder_inputs = torch.tensor([candidate_seq[-1]]).unsqueeze(0).to(model.device)
            
                output_logits, decoder_hidden_state = model(seqx, decoder_inputs, decoder_hidden_state)      
                output_logits = output_logits.squeeze(1)
                topk_probs, topk_indices = torch.topk(output_logits, k, dim=-1)       
        
                for i in range(k):
   
                    new_seq = candidate_seq + [topk_indices[0][i].item()]

                    new_probs = candidate_probs + topk_probs[0][i].item()
                    
                    new_candidates.append((new_probs,new_seq,decoder_hidden_state))
                    # length_penalty = math.pow((5 + len(new_seq)) / 6, length_penalty_alpha)
                    # new_candidates.append((new_probs / length_penalty, new_seq, decoder_hidden_state))
                   
                   
       
            new_candidates = sorted(new_candidates, key = lambda x: x[0], reverse = True)
        
            beam_candidates = new_candidates[:k]
    
    return tgt_tokenizer.decode(beam_candidates[0][1][:])
